  ...
  <model name="octorotor">
    ...
    <link name="base_link">
      ...
      <visual name="base_link_visual">
        ...
        <material><ambient>1 1 1 1</ambient><diffuse>1 1 1 1</diffuse><specular>0.8 0.8 0.8 1</specular></material>
      </visual>

      <!-- Arm Front (+x) -->
      <visual name="arm_front">
        <pose>{Ld/2} 0 0 0 1.5708 0</pose>
        <geometry><cylinder><radius>0.015</radius><length>{Ld}</length></cylinder></geometry>
...
positions = [
    (Ld, 0, 0.05, 'cw', 0, '1 1 1 0.8'),
    (Ld, 0, -0.05, 'ccw', 1, '1 1 1 0.8'),
    (0, Ld, 0.05, 'ccw', 2, '1 1 1 0.8'),
    (0, Ld, -0.05, 'cw', 3, '1 1 1 0.8'),
    (-Ld, 0, 0.05, 'cw', 4, '1 1 1 0.8'),
    (-Ld, 0, -0.05, 'ccw', 5, '1 1 1 0.8'),
    (0, -Ld, 0.05, 'ccw', 6, '1 1 1 0.8'),
    (0, -Ld, -0.05, 'cw', 7, '1 1 1 0.8')
]

for x, y, z, dir, idx, color in positions:
    sdf += f'''
    <link name="rotor_{idx}">
      ...
      <visual name="rotor_{idx}_visual">
        <geometry>
          <cylinder><radius>0.12</radius><length>0.01</length></cylinder>
        </geometry>
